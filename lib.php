...
function theme_hskit_get_main_scss_content($theme) {                                                                                
    global $CFG;                                                                                                                    
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_hskit', 'preset', 0, '/', $filename))) {              
        // This preset file was fetched from the file area for theme_hskit and not theme_boost (see the line above).                
...
function theme_hskit_update_settings_images($settingname) {                                                                         
    global $CFG;                                                                                                                    
 
    // The setting name that was updated comes as a string like 's_theme_hskit_loginbackgroundimage'.                               
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {                                                                           
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/hskit/' . $settingname . '.' . $extension;                                 
 
        // This pattern matches any previous files with maybe different file extensions.                                            
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/hskit/' . $settingname . '.*';                                          
 
        // Make sure this dir exists.                                                                                               
        @mkdir($CFG->dataroot . '/pix_plugins/theme/hskit/', $CFG->directorypermissions, true);                                      
